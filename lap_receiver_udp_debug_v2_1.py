...
class TransmitterState:
    ...
    parallel_reject_active: bool = False
...
def evaluate_gate(
    ...
    if (
        ...
        combined_rssi = min(rx1_avg, rx2_avg)
        rssi_difference = abs(rx1_avg - rx2_avg)
        gate_valid = rssi_difference <= MAX_RECEIVER_RSSI_DIFFERENCE
        entry_signal = (
            gate_valid
            and rx1_avg >= ENTRY_RSSI_THRESHOLD
            and rx2_avg >= ENTRY_RSSI_THRESHOLD
        )
    ...
    if parallel_now and not state.parallel_reject_active:
        parallel_reject_count += 1
        state.parallel_reject_active = True
        print()
        print(
            f"[並走除外] {transmitter.serial_number} "
            f"RX1={rx1_avg} RX2={rx2_avg} 差={rssi_difference} dB"
        )
    elif not parallel_now:
        state.parallel_reject_active = False
    ...
    if state.gate_state == GateState.WAIT:
        if entry_signal:
            ...
            if DEBUG_ENTRY:
                print()
                print(
                    f"[統合ENTRY開始] {transmitter.serial_number} "
    ...
    if state.gate_state == GateState.ENTRY_CANDIDATE:
        if not entry_signal:
            ...
            if DEBUG_ENTRY:
                print()
                print(f"[統合ENTRYキャンセル] {transmitter.serial_number} 経過={elapsed:.2f}秒")
        ...
        if DEBUG_ENTRY:
            print()
            print(
                f"[統合ENTRY成立] {transmitter.serial_number} "
    ...
    if state.gate_state == GateState.INSIDE:
        if both_below_exit:
            ...
            state.exit_candidate_since = now_monotonic
            print()
            print(f"[統合EXIT候補] {transmitter.serial_number}")
        return
    ...
    if state.gate_state == GateState.EXIT_CANDIDATE:
        if not both_below_exit:
            ...
            state.gate_state = GateState.INSIDE
            print()
            print(f"[統合EXITキャンセル] {transmitter.serial_number}")
            return
        ...
        if now_monotonic - state.exit_candidate_since >= EXIT_CONFIRM_SECONDS:
            ...
            state.gate_state = GateState.WAIT
            print()
            print(f"[統合ゾーン離脱] {transmitter.serial_number}")
            print("次の通過を受け付けます。")
...
class SgsUdpProtocol(asyncio.DatagramProtocol):
    ...
    def connection_made(
        ...
        print()
        print("============================================")
        print(" SGS UDP Lap Receiver Version 2.1")
        print("============================================")
        print(
            f"[UDP] Listening on "
